...
func generate_maze() -> void:
	...
	for m in numberOfMonsters:
		...
		var monsterx = randi_range(0, maze_size)
		var monstery = randi_range(0, maze_size-2)
...
func randomInnerWalls():
	for i in round(pow(maze_size, 1.5)):
		addWallToScene(true, randi_range(0,maze_size-1), randi_range(1,maze_size-1))
		addWallToScene(false, randi_range(0,maze_size-1), randi_range(1,maze_size-1))

func addWallToScene(horizontal : bool, x, y) -> Node3D:
...
func calculateTilePosition(x,y) -> Vector3:
	var tilex = (x - (maze_size/2.0)) * wallWidth
	var tiley = (y - (maze_size/2.0)) * wallWidth
